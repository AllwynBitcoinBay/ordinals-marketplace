...
const ItemViewWallet: React.FC<ItemProps> = (
    ...
    useEffect(() => {
        if (item.origin.data.insc.file.type === "text/plain") {
            const url = `https://ordinals.gorillapool.io/content/${item.origin.outpoint}`;
            fetch(url)
    ...
    <Card sx={
      ...
      <CardContent style={{
        ...
        {item.origin.data.insc.file.type.startsWith('image') && (
          <img style={{ maxWidth: 250 }} src={`https://ordinals.gorillapool.io/content/${item.origin.outpoint}`} alt={`Content #${item.origin.num}`} />
        )}
